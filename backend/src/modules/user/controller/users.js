...
exports.projectsDetails = async (req, res) => {
  try {
    ...
    if (userObj.acceptedProjects.includes(req.params.projectId)) {
      return res.status(constants.STATUS_CODE.SUCCESS_STATUS).send({
        status: 'Accepted',
      });
    }
    if (userObj.rejectedProjects.includes(req.params.projectId)) {
      return res.status(constants.STATUS_CODE.SUCCESS_STATUS).send({
        status: 'Rejected',
      });
    }
...
exports.joinProject = async (req, res) => {
  try {
    const projectObj = await Projects.findById(req.body.projectId);
    if (projectObj.requestedTesters.includes(req.body.userId)) {
      return res.status(constants.STATUS_CODE.NO_CONTENT_STATUS).send('Already a member');
    }
    ...
    return res.status(constants.STATUS_CODE.SUCCESS_STATUS).send('Success');
  } catch (error) {
    return res.status(constants.STATUS_CODE.INTERNAL_SERVER_ERROR_STATUS).send(error.message);
  }
};
